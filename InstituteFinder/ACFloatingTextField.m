...
-(void)initialization{
    ...
    if (_placeHolderTextColor == nil){
        _placeHolderTextColor = [UIColor blueColor];
    }
    
    //2. Placeholder Color When Selected.
    if (_selectedPlaceHolderTextColor==nil) {
        _selectedPlaceHolderTextColor = [UIColor redColor];
    }
    
    //3. Bottom line Color.
    if (_btmLineColor==nil) {
        _btmLineColor = [UIColor redColor];
    }
    ...
    bottomLineView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetHeight(self.frame)-1, CGRectGetWidth(self.frame), 20)];
